...
def _minutes_to_time(m: int) -> time:
    """Convert minutes since midnight back to a time-of-day."""
    return time(hour=m // 60, minute=m % 60)


def _intersect(a_start, a_end, b_start, b_end):
    """Return intersection of two time windows in minutes."""
...
def suggest_slots(
    ...
    """
    Generate up to n deterministic meeting slot suggestions.

    The function enforces the following system constraints:

    • Deterministic outputs under identical inputs
    • Conflict-free scheduling
    • Chronological ordering of slots
    • Respect for working hours, busy intervals, buffers, and candidate window
    """
    ...
    # ---------------------------------------------------------
    # Step 2 — Determine effective scheduling window
    # ---------------------------------------------------------

    work_start = _time_to_minutes(working_hours.start)
    work_end = _time_to_minutes(working_hours.end)

    # If candidate window exists, intersect it with working hours
    if candidate_window:
        cand_start = _time_to_minutes(candidate_window.start)
        cand_end = _time_to_minutes(candidate_window.end)

        window = _intersect(work_start, work_end, cand_start, cand_end)

        # No overlap between windows → no scheduling possible
        if window is None:
            return []
    ...
    # ---------------------------------------------------------
    # Step 3 — Normalize busy intervals
    # ---------------------------------------------------------
    ...
    for b in busy_intervals:
        ...
        # Clip interval to scheduling window
        clipped = _intersect(b_start, b_end, sched_start, sched_end)
        if not clipped:
            continue

        s, e = clipped

        # Apply buffer before and after busy interval
        s -= buffer_min
        e += buffer_min

        # Clip again after buffer expansion
        s = max(s, sched_start)
        e = min(e, sched_end)
    ...
    # ---------------------------------------------------------
    # Step 4 — Sort intervals to ensure deterministic ordering
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Step 6 — Compute available scheduling gaps
    # ---------------------------------------------------------
    ...
    # Gap after the final busy interval
    if prev_end < sched_end:
        gaps.append((prev_end, sched_end))

    # ---------------------------------------------------------
    # Step 7 — Generate meeting slots from gaps
    # ---------------------------------------------------------
    ...
    for g_start, g_end in gaps:
        ...
        # Slide start time forward deterministically
        while start + duration_min <= g_end:

            results.append(Slot(start_time=_minutes_to_time(start)))

            # Respect maximum number of suggestions
            if len(results) >= n:
                return results

            start += 1  # deterministic 1-minute step

    # ---------------------------------------------------------
    # Step 8 — Edge case: no valid slots
    # ---------------------------------------------------------
